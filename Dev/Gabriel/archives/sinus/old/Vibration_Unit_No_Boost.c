/* * File:   main.c (sinus-LUT drop-in, NO BOOST)
 * Note:    Duty uses 5 bits (0..31) mapped to 0..99, waveform = sine via LUT
 */


// Map 5-bit (0..31) to 0..99 with rounding ()
//  0→0,  1→3,  2→6,  3→10, 4→13, 5→16, 6→19, 7→22,
// 8→26, 9→29, 10→32, 11→35, 12→38, 13→42, 14→45, 15→48,
// 16→51, 17→54, 18→57, 19→61, 20→64, 21→67, 22→70, 23→73,
// 24→77, 25→80, 26→83, 27→86, 28→89, 29→93, 30→96, 31→99
...
static void init_ccp_cwg(void){
    ...
    // CWG comme la version qui marche
    ...
    CWG1DBR    = 0;          // deadband off (inchangé)
    CWG1DBF    = 0;
...
// ---------- PWM/Bridge drive (SINUS LUT, CWG intact) ----------
static void CCP_processing(void){
    // Enveloppe sinus (0..99 → 0..100 %) pour 0..199
    ...
    if( (index < duty_cycle) || (index >= 100 && index < (uint8_t)(100 + duty_cycle)) ){
        ...
        // OFF → sorties identiques (diff=0) + quasi zéro PWM
        if (cwg_flag == 1){
            CWG1CON0bits.EN = 0;
            CWG1CON1bits.POLB = 1;  // "same"
            CWG1CON0bits.EN = 1;
            cwg_flag = 0;
        }
        CCPR1H = 0x00;
