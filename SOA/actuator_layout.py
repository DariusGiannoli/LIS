...
Version avec Canvas élargi pour plus d'espace de travail et vibration au survol
"""
import sys
import math
from PyQt6.QtWidgets import (
    ...
    QLineEdit, QComboBox, QApplication, QMainWindow
)
...
# Import your API - make sure this is in the same directory
try:
    from python_serial_api import python_serial_api
except ImportError:
    print("Warning: python_serial_api not found. Vibration will not work.")
    python_serial_api = None

class SelectableActuator(QGraphicsItem):
    """Actuateur sélectionnable et supprimable avec vibration au survol"""
    ...
    def paint(self, painter, option, widget=None):
        ...
        if self.vibrating:
            painter.setBrush(QBrush(QColor(255, 100, 100)))  # Rouge pour vibration active
            painter.setPen(QPen(QColor(200, 50, 50), 3))
        elif self.hovered:
            painter.setBrush(QBrush(QColor(255, 165, 0)))  # Orange pour hover
    ...
    def show_context_menu(self, event):
        ...
        test_vibration_action = QAction("Test Vibration", menu)
        menu.addAction(test_vibration_action)
        menu.addAction(delete_action)
        
        choice = menu.exec(event.screenPos().toPoint())
        if choice == delete_action:
            self.remove_self()
        elif choice == test_vibration_action:
            self.test_vibration()
    
    def test_vibration(self):
        """Tester la vibration de cet actuateur"""
        view = self.scene().views()[0] if self.scene() and self.scene().views() else None
        if view and view.api and view.api.connected:
            print(f"Testing vibration for actuator {self.actuator_id}")
            # Test vibration for 1 second
            view.api.send_command(self.actuator_id, 10, 5, 1)  # Start with medium intensity
            # Note: In a real application, you'd want to stop after a delay
        else:
            print(f"Cannot test vibration - no API connection")
    ...
    def start_vibration(self):
        ...
        view = self.scene().views()[0] if self.scene() and self.scene().views() else None
        if view and view.api and view.api.connected:
            # Use preview vibration settings from the main widget
            intensity = view.hover_intensity
            frequency = view.hover_frequency
            
            success = view.api.send_command(self.actuator_id, intensity, frequency, 1)
            if success:
                self.vibrating = True
                self.update()
                print(f"Started vibration for actuator {self.actuator_id} (intensity: {intensity}, freq: {frequency})")
            else:
                print(f"Failed to start vibration for actuator {self.actuator_id}")
        else:
            print(f"Cannot start vibration - no API connection")
    ...
    def stop_vibration(self):
        ...
        if self.vibrating:
            view = self.scene().views()[0] if self.scene() and self.scene().views() else None
            if view and view.api and view.api.connected:
                success = view.api.send_command(self.actuator_id, 0, 0, 0)
                if success:
                    print(f"Stopped vibration for actuator {self.actuator_id}")
                else:
                    print(f"Failed to stop vibration for actuator {self.actuator_id}")
...
class FlexibleActuatorView(QGraphicsView):
    ...
    def __init__(self):
        ...
        self.next_id = 0
        self.api = None  # API de contrôle des actuateurs physiques
        
        # Default hover vibration settings
        self.hover_intensity = 8
        self.hover_frequency = 4
    ...
    def set_api(self, api):
        """Définir l'API pour contrôler les actuateurs physiques"""
        self.api = api
        print(f"API set: connected = {api.connected if api else False}")
    
    def set_hover_settings(self, intensity, frequency):
        """Set the hover vibration settings"""
        self.hover_intensity = intensity
    ...
    def clear_canvas(self):
        """Vider le canvas"""
        # Stop all vibrations before clearing
        for actuator in self.actuators:
            actuator.stop_vibration()
...
class FlexibleActuatorSelector(QWidget):
    """Widget principal avec palette et contrôles - Canvas élargi et contrôles de vibration"""
    ...
    def setup_api(self):
        """Initialiser l'API série"""
        if python_serial_api:
            self.api = python_serial_api()
            self.canvas.set_api(self.api)
            # Set initial hover settings
            self.canvas.set_hover_settings(8, 4)  # Default values
            self.refresh_devices()
        else:
            print("Serial API not available - vibration features disabled")
    ...
    def setup_ui(self):
        ...
        title_label = QLabel("Actuator Designer with Vibration")
        ...
        # Hover vibration settings
        hover_layout = QHBoxLayout()
        hover_layout.setSpacing(5)
        
        hover_layout.addWidget(QLabel("Hover Vibration - Intensity:"))
        self.hover_intensity_spin = QSpinBox()
        self.hover_intensity_spin.setRange(0, 15)
        self.hover_intensity_spin.setValue(8)
        self.hover_intensity_spin.setMaximumWidth(50)
        hover_layout.addWidget(self.hover_intensity_spin)
        
        hover_layout.addWidget(QLabel("Frequency:"))
        self.hover_frequency_spin = QSpinBox()
        self.hover_frequency_spin.setRange(0, 7)
        self.hover_frequency_spin.setValue(4)
        self.hover_frequency_spin.setMaximumWidth(50)
        hover_layout.addWidget(self.hover_frequency_spin)
        ...
        # Connect hover settings changes
        self.hover_intensity_spin.valueChanged.connect(self.update_hover_settings)
        self.hover_frequency_spin.valueChanged.connect(self.update_hover_settings)
    ...
    def update_hover_settings(self):
        """Update hover vibration settings in the canvas"""
        self.canvas.set_hover_settings(
            self.hover_intensity_spin.value(),
            self.hover_frequency_spin.value()
        )
    ...
    def refresh_devices(self):
        ...
        if devices:
            ...
            self.status_label.setText(f"Found {len(devices)} serial devices")
        else:
            self.status_label.setText("No serial devices found")
    
    def toggle_connection(self):
        """Connect or disconnect from the selected device"""
        if not self.api:
            QMessageBox.warning(self, "Error", "Serial API not available")
            return
        ...
        if not self.api.connected:
            ...
            if self.device_combo.currentText():
                if self.api.connect_serial_device(self.device_combo.currentText()):
                    ...
                    self.status_label.setText("Connected to device - hover over actuators for vibration")
                else:
                    QMessageBox.warning(self, "Connection Failed", "Failed to connect to the selected device")
            else:
                QMessageBox.warning(self, "No Device", "Please select a device to connect to")
        else:
            # Disconnect
        ...
        if selected_ids:
            ...
            if total > 0:
                if self.api and self.api.connected:
                    self.status_label.setText(f"{total} actuators - none selected - hover for vibration")
                else:
...
if __name__ == "__main__":
    ...
    window = QMainWindow()
    window.setWindowTitle("Flexible Actuator Selector with Vibration - Enhanced")
