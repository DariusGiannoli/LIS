...
class DirectionStudyInterface:
    """
    GUI Interface for Direction Study - Tactile Directional Perception
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Direction Study Interface - Enhanced")
    ...
    def setup_gui(self):
        ...
        self.title_label = ttk.Label(
            header_frame, 
            text="Direction Study - Tactile Directional Perception (Enhanced)",
            font=('Arial', 16, 'bold')
        )
    ...
    def create_study_sequence(self):
        ...
        print(f"Motion-only random order: {[f'{direction}_{pattern}' for direction, pattern in combined_random_order]}")
...
- All pattern types (static, pulse, motion) mixed together [Currently: Motion only]
...
- Motion patterns created smooth directional movement
...
- 'New Study' to start a fresh study with new randomization"""
    ...
    def save_results(self):
        ...
        filename = f"direction_study_results_{self.participant_id}_{timestamp}"
