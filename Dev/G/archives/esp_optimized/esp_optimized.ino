...
#define MIN_IFG_US        120    // inter-frame gap per subchain (~>1 char @115200)
...
// Last TX time per subchain (for IFG guard)
...
static inline void sendStop(int group, int addr) {
  ...
  for (int k = 0; k < STOP_RETRIES; ++k) {
    guard_ifg(group);                   // ensure a small gap on this subchain
...
static inline void sendStart3(int group, int addr, uint8_t duty5, uint8_t freq) {
  ...
  guard_ifg(group);                   // small inter-frame gap per subchain
  serial_group[group].write(msg, 3);
  serial_group[group].flush();
  // Optionnel: ajouter un très petit délai si tu enchaînes vers la même adresse
...
static inline void processFrame(const uint8_t *f) {
  ...
  // Fast path: full padding frame -> drop
  if (b1 == 0xFF && b2 == 0xFF && b3 == 0xFF && b4 == 0xFF) return;
...
void loop() {
  ...
  while (rxLen >= 4) {
    // Drop a full padding frame quickly (FF FF FF FF)
    if (rxBuf[0] == 0xFF && rxBuf[1] == 0xFF && rxBuf[2] == 0xFF && rxBuf[3] == 0xFF) {
