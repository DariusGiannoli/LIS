...
// Packet size - updated for timed commands (20 commands × 5 bytes = 100 bytes)
...
void setup() {
  ...
  Serial.println("Master ESP32 - Serial to ESP-Now Bridge (Timed Commands)");
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if ((firstByte >= 'a' && firstByte <= 'z') || (firstByte >= 'A' && firstByte <= 'Z')) {
      ...
      if (input == "test") {
        Serial.println("Sending test timed command to slave...");
        
        // Create a test command packet with timing
        ...
        // Create test command for motor addr=1, duty=7, freq=2, start, delay=0ms
        int addr = 1;
        int duty = 7;
        int freq = 2;
        int start_or_stop = 1;
        ...
        testData[3] = delay_ms & 0xFF;          // Delay low byte
        testData[4] = (delay_ms >> 8) & 0xFF;   // Delay high byte
        
        Serial.println("Test timed command created:");
        Serial.printf("Bytes: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X (delay=%dms)\n", 
                      testData[0], testData[1], testData[2], testData[3], testData[4], delay_ms);
        
        // Send via ESP-Now
        esp_err_t result = esp_now_send(slaveAddress, testData, PACKET_SIZE);
        
        if (result == ESP_OK) {
          Serial.println("ESP-Now: Test timed command queued for transmission");
          strip.setPixelColor(0, strip.Color(255, 255, 0)); // Yellow
          strip.show();
        } else {
          Serial.println("ESP-Now: Error sending test timed command");
  ...
  // Wait for at least some data, then try to collect full packet
  if (Serial.available() > 0) {
    ...
    if (availableBytes >= PACKET_SIZE) {
      ...
      size_t bytesRead = Serial.readBytes(data, PACKET_SIZE);
      ...
      if (bytesRead == PACKET_SIZE) {
        ...
        if (result == ESP_OK) {
          Serial.println("ESP-Now: Python timed data queued for transmission");
          // Yellow LED during transmission
          strip.setPixelColor(0, strip.Color(255, 255, 0));
          strip.show();
        } else {
          Serial.println("ESP-Now: Error sending Python timed data");
          // Red LED for error
          strip.setPixelColor(0, strip.Color(255, 0, 0));
          strip.show();
        }
