...
// outArray must have space for 4 bytes (32 bits).
...
void encodeByte(uint8_t inputByte, uint8_t outArray[4])
{
    ...
    // Now we have 24 bits in 'encoded'. Split into 4 bytes
    // The top 8 bits are (encoded >> 24), then >> 16, then >> 8, then low 8 bits
...
void sendColor_SPI(uint8_t g, uint8_t r, uint8_t b)
{
    ...
    // Total 12 bytes to send for 24 bits color
    // G -> 4 bytes, R -> 4 bytes, B -> 4 bytes
