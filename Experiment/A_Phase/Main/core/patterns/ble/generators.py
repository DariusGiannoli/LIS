...
class BLEOptimizedMotionEngine:
    """
    BLE-Optimized motion pattern engine using Park et al. (2016) algorithms
    
    Keeps the Park et al. algorithm intact but optimizes output for Bluetooth:
    - Command batching to reduce BLE calls
    - Timing optimization for BLE latency
    - Smart grouping of simultaneous commands
    """
    ...
    def __init__(self):
        ...
        # BLE optimization parameters
        self.BLE_COMMAND_WINDOW_MS = 50  # Group commands within 50ms windows
        self.BLE_MIN_DELAY_MS = 20       # Minimum delay between BLE calls
        self.MAX_COMMANDS_PER_BATCH = 10 # Max commands per send_command_list call
    ...
    def create_phantom(self, position, intensity):
        ...
        for actuator_id, actuator_pos in self.actuators.items():
            if self._distance(position, actuator_pos) < 1.0:
                if self.DEBUG:
    ...
    def optimize_commands_for_ble(self, motion_commands, freq=FREQ):
        """
        Optimize motion commands for BLE transmission
        
        BLE Optimizations:
        1. Group commands into time windows to reduce API calls
        2. Batch simultaneous commands using send_command_list
        3. Ensure minimum delays between BLE transmissions
        4. Limit commands per batch to prevent BLE buffer overflow
        """
        if not motion_commands:
            return {'steps': []}
        
        # Create all start/stop events
        events = []
        ...
        for cmd in motion_commands:
            if cmd.actuator_id in available_devices:
                ...
                # Start event
                events.append({
                    'time_ms': int(cmd.onset_time * 1000),
                    'type': 'start',
                    'command': {
                ...
                # Stop event
                events.append({
                    'time_ms': int((cmd.onset_time + cmd.duration) * 1000),
                    'type': 'stop',
                    'command': {
                        "addr": cmd.actuator_id,
                        "duty": 0,
        ...
        # Group events into BLE-optimized time windows
        steps = []
        current_time_ms = 0
        i = 0
        
        while i < len(events):
            # Find all events within the current BLE command window
            window_start = events[i]['time_ms']
            window_commands = []
            
            # Collect events within the BLE command window
            while (i < len(events) and 
                   events[i]['time_ms'] <= window_start + self.BLE_COMMAND_WINDOW_MS and
                   len(window_commands) < self.MAX_COMMANDS_PER_BATCH):
                window_commands.append(events[i]['command'])
                i += 1
            
            # Calculate delay from current time to window start
            delay_ms = max(0, window_start - current_time_ms)
            
            # Ensure minimum delay between BLE calls (except first step)
            if steps and delay_ms < self.BLE_MIN_DELAY_MS:
                delay_ms = self.BLE_MIN_DELAY_MS
            
            # Update the delay of the previous step
            if steps and delay_ms > 0:
                steps[-1]['delay_after_ms'] = delay_ms
            
            # Add the batched commands step
            steps.append({
                'commands': window_commands,
                'delay_after_ms': 0  # Will be set by next iteration or remain 0 for last
            })
            
            current_time_ms = window_start + delay_ms
        
        if self.DEBUG:
            total_ble_calls = len(steps)
            total_commands = sum(len(step['commands']) for step in steps)
            print(f"BLE Optimization: {total_commands} commands grouped into {total_ble_calls} BLE calls")
            print(f"Average commands per BLE call: {total_commands/total_ble_calls:.1f}")
        
        return {'steps': steps}

# Global BLE-optimized motion engine
_ble_motion_engine = None

def get_ble_motion_engine():
    global _ble_motion_engine
    if _ble_motion_engine is None:
        _ble_motion_engine = BLEOptimizedMotionEngine()
    return _ble_motion_engine

# ================================
# BLE-OPTIMIZED PATTERN FUNCTIONS
# ================================
...
def generate_ble_static_pattern(devices, duty=DUTY, freq=FREQ, duration=DURATION):
    """Generate BLE-optimized static vibration pattern"""
    duties = duty if isinstance(duty, list) else [duty] * len(devices)
    
    # Single batched start command
    start_commands = [
        {
            "addr": addr,
            "duty": d,
...
def generate_ble_pulse_pattern(devices, duty=DUTY, freq=FREQ, pulse_duration=PULSE_DURATION, 
                              pause_duration=PAUSE_DURATION, num_pulses=NUM_PULSES):
    """Generate BLE-optimized pulsed vibration pattern"""
    duties = duty if isinstance(duty, list) else [duty] * len(devices)
    
    steps = []
    
    for pulse_num in range(num_pulses):
        # Batched start commands for all devices
        start_commands = [
            {
                "addr": addr,
                "duty": d,
        ...
        # Batched stop commands for all devices
        stop_commands = [
            {
                "addr": addr,
                "duty": 0,
...
def generate_ble_motion_pattern(devices, intensity=DUTY, freq=FREQ, duration=0.04, 
                               movement_speed=MOVEMENT_SPEED, max_sampling_interval=0.05, **kwargs):
    """
    Generate BLE-optimized motion pattern using Park et al. (2016) algorithm
    
    This function:
    1. Uses the exact Park et al. algorithm for phantom sensation generation
    2. Optimizes the output for BLE transmission with command batching
    3. Reduces BLE API calls while maintaining motion quality
    """
    
    engine = get_ble_motion_engine()
    ...
    # Generate motion commands using Park et al. algorithm (unchanged)
    ...
    # Apply BLE optimizations to the motion commands
    return engine.optimize_commands_for_ble(motion_commands, freq)

def generate_ble_sequential_pattern(devices, duty=DUTY, freq=FREQ, duration_per_device=500, pause_between=100):
    """Generate BLE-optimized sequential activation pattern"""
    steps = []
    
    for i, addr in enumerate(devices):
        # Individual commands for sequential activation
        start_command = {
            "addr": addr,
            "duty": duty,
            "freq": freq,
...
def generate_ble_coordinate_pattern(coordinates, intensity=DUTY, freq=FREQ, duration=MOTION_DURATION, 
                                   movement_speed=MOVEMENT_SPEED):
    """
    Generate BLE-optimized coordinate-based motion patterns
    
    Main entry point that automatically chooses the best pattern type
    and applies BLE optimizations
    """
    ...
    if all(isinstance(item, int) for item in coordinates):
        ...
        pause_ms = max(50, int(duration_ms * 0.2))
        return generate_ble_sequential_pattern(
            devices=coordinates,
            duty=intensity,
            freq=freq,
    ...
    # Mixed coordinates or pure coordinates - use BLE-optimized Park et al. motion
    return generate_ble_motion_pattern(
        devices=coordinates,
        intensity=intensity,
        freq=freq,
        duration=duration,
...
# Keep original function names for compatibility, but use BLE-optimized versions
def generate_static_pattern(*args, **kwargs):
    """Backwards compatible - uses BLE-optimized version"""
    return generate_ble_static_pattern(*args, **kwargs)

def generate_pulse_pattern(*args, **kwargs):
    """Backwards compatible - uses BLE-optimized version"""
    return generate_ble_pulse_pattern(*args, **kwargs)

def generate_motion_pattern(*args, **kwargs):
    """Backwards compatible - uses BLE-optimized version"""
    return generate_ble_motion_pattern(*args, **kwargs)

def generate_sequential_pattern(*args, **kwargs):
    """Backwards compatible - uses BLE-optimized version"""
    return generate_ble_sequential_pattern(*args, **kwargs)

def generate_coordinate_pattern(*args, **kwargs):
    """Backwards compatible - uses BLE-optimized version"""
    return generate_ble_coordinate_pattern(*args, **kwargs)