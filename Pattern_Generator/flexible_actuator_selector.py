...
class SelectableActuator(QGraphicsItem):
    ...
    def hoverEnterEvent(self, event):
        ...
        # Activer la vibration physique si API disponible
        view = self.scene().views()[0] if self.scene() and self.scene().views() else None
        if view and view.api and view.api.connected:
            # Utiliser des valeurs fixes pour le preview (intensity=8, freq=4, start=1)
            view.api.send_command(self.actuator_id, 8, 4, 1)
    ...
    def hoverLeaveEvent(self, event):
        ...
        # Arrêter la vibration physique si API disponible
        view = self.scene().views()[0] if self.scene() and self.scene().views() else None
        if view and view.api and view.api.connected:
            view.api.send_command(self.actuator_id, 0, 0, 0)
...
class FlexibleActuatorView(QGraphicsView):
    ...
    def __init__(self):
        ...
        self.next_id = 0
        self.api = None  # Ajout pour l'API de contrôle des actuateurs physiques
