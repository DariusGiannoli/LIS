...
class python_ble_api:
    ...
    def create_command(self, addr, duty, freq, start_or_stop):
        """
        Nouvelle trame 4 octets (duty 5 bits):
          Byte1: [GGGG][0][S]              (group = addr//16, S=start/stop)
          Byte2: [01][ADDR6] = 0x40 | (addr%16)
          Byte3: [1][-----DUTY5] = 0x80 | (duty & 0x1F)   (0..31)
          Byte4: [1][-----FREQ3] = 0x80 | (freq & 0x07)   (0..7)
        """
        serial_group = int(addr) // 16
        serial_addr  = int(addr) % 16

        duty5 = int(duty) & 0x1F          
        freq3 = int(freq) & 0x07          
        sos   = int(start_or_stop) & 0x01

        byte1 = (serial_group << 2) | sos
    ...
    async def send_command_async(self, addr, duty, freq, start_or_stop) -> bool:
        ...
        if addr < 0 or addr > 127 or duty < 0 or duty > 31 or freq < 0 or freq > 7 or start_or_stop not in [0, 1]:
            return False
        ...
        try:
            ...
            print(f'BLE sent command to #{addr} with duty5 {duty} and freq {freq}, start_or_stop {start_or_stop}')
            return True
        except Exception as e:
            print(f'BLE failed to send command to #{addr} with duty5 {duty} and freq {freq}. Error: {e}')
            return False
    ...
    async def send_command_list_async(self, commands) -> bool:
        ...
        for c in commands:
            ...
            if addr < 0 or addr > 127 or duty < 0 or duty > 31 or freq < 0 or freq > 7 or start_or_stop not in [0, 1]:
                return False
            command += self.create_command(int(addr), int(duty), int(freq), int(start_or_stop))
        # padding à 80 octets: 20 trames * 4 octets
        frames = len(command) // 4
        if frames < 20:
            command += bytearray([0xFF, 0xFF, 0xFF, 0xFF]) * (20 - frames)
        try:
            await self.client.write_gatt_char(self.MOTOR_UUID, command)
...
if __name__ == '__main__':
    ...
    if 'QT Py ESP32-S3' in device_names:
        if ble_api.connect_ble_device('QT Py ESP32-S3'):
            # START duty5=15 (~50%), freq=3
            ble_api.send_command(1, 15, 3, 1)
            time.sleep(1.5)
            # STOP
            ble_api.send_command(1, 15, 3, 0)
            time.sleep(1)
            # START duty5=25 (~80%), STOP
            ble_api.send_command(1, 25, 3, 1)
            time.sleep(1.0)
            ble_api.send_command(1, 25, 3, 0)
            time.sleep(1.0)
