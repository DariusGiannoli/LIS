...
# Only motion vertical for testing
...
print(f"Vertical combined order: {[p['name'] for p in vertical_combined]}")

def emergency_stop_all(api):
    """Send stop commands to ALL 16 actuators - simplified version"""
    all_stop_commands = [
...
def execute_motion_pattern_smooth(api, pattern):
    """Execute motion pattern with minimal interruption for smooth playback"""
    ...
    try:
        ...
        # Single final stop for activated actuators only
...
def safe_motion_execution(api, pattern):
    """Execute motion pattern with optimized safety measures"""
    try:
        # Pre-execution clean stop (minimal)
        emergency_stop_all(api)
        time.sleep(0.1)  # Reduced delay
        
        # Execute the pattern smoothly
        execute_motion_pattern_smooth(api, pattern)
...
def run_vertical_motion_patterns(api, patterns):
    """Run vertical motion patterns with optimized execution"""
    print(f"\n=== VERTICAL MOTION PATTERNS ===")
    print(f"Total patterns: {len(patterns)}")

    idx = 0
    while idx < len(patterns):
        pattern_info = patterns[idx]
        pattern = pattern_info['pattern']
        current_num = idx + 1

        print(f"\nPlaying {pattern_info['name']} ({current_num} of {len(patterns)})")
        print(f"Pattern has {len(pattern['steps'])} steps")
        
        try:
            # Execute pattern with optimized safety measures
            safe_motion_execution(api, pattern)
            time.sleep(sleep_during)

            action = wait_for_input(pattern_info['name'], current_num, len(patterns))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if api.connect_serial_device(ports[2]):
            ...
            print("=== VERTICAL MOTION STUDY ===")
            print("Controls: 'r' = repeat, 'n' = next, 'q' = quit, 's' = emergency stop")
            print(f"Total motion patterns: {len(vertical_combined)}")
            print("Optimized for smooth motion playback")
            
            # Run vertical motion patterns
            result = run_vertical_motion_patterns(api, vertical_combined)
            
            if result == 'completed':
                print("\n=== VERTICAL MOTION STUDY COMPLETED ===")
            else:
                print("\n=== STUDY TERMINATED ===")
